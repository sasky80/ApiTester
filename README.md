...
- Headers:
  - The headers table always contains a first row named `Content-Type`. Edit its *value* to change the content type. If you type a value that is not one of the built-in types (`application/json`, `application/xml`, `text/plain`) the UI will automatically select `Other` and populate the custom content-type textbox.
